...
struct esp32p4_soc {
    ...
    /* flash cache MMU page table: 1024 x 64KB pages (64 MB window),
     * programmed through SPI_MEM_C_MMU_ITEM_CONTENT (+0x37C, translated
     * SPI0 block). Entries default to invalid (~0u); unmapped window
     * reads fall back to the mask ROM (low 128 KB) or zero. */
    uint32_t mmu[1024];
    uint32_t mmu_page_mode; /* P4 has no PAGE_MODE reg; fixed 64KB pages */
     ...
     * frequency queries compute sane values. HW-set status bits are
     ...
     * completion, like the AHB AES path). */
    uint32_t axi_out_link2[3];   /* OUT descriptor address (full) */
    uint32_t axi_out_peri_sel[3];
    uint32_t axi_out_int_raw[3];
    uint32_t axi_out_int_ena[3];
    uint32_t axi_in_link2[3];    /* IN descriptor address (full) */
    uint32_t axi_in_peri_sel[3];
    uint32_t axi_in_int_raw[3];
    uint32_t axi_in_int_ena[3];
    /* P4 L1/L2 cache control (0x3FF10000): 1KB of plain storage; sync /
     * autoload DONE bits read back as complete so ROM/SDK cache ops
     * finish instantly. */
    uint32_t cache_reg[256];
};
...
static void esp32p4_gdma_load_desc(esp32p4_t *soc, int ch)
{
    uint8_t *d = esp32p4_dma_ptr(soc, soc->gdma_tx_desc_addr[ch]);
...
static void esp32p4_gdma_load_rx_desc(esp32p4_t *soc, int ch)
{
    uint8_t *d = esp32p4_dma_ptr(soc, soc->gdma_rx_desc_addr[ch]);
    if (!d) { /* descriptor not in RAM: park the channel */
...
static void esp32_add_region(esp32p4_t *soc,
                             uint32_t base,
                             uint32_t size,
                             esp32_reg_type_t type)
{
...
esp32p4_t *esp32p4_new(void)
{
    ...
    esp32_add_region(soc, P4_LP_SRAM_BASE, P4_LP_SRAM_SIZE, ESP32_REG_RAM);

    /* flash cache window (shares its address range with the mask ROM:
     * unmapped reads serve the ROM image, see esp32_flash_window_off) */
    esp32_region_t *fw = &soc->regions[soc->nregions++];
    fw->base = P4_FLASH_I_BASE;
    fw->size = P4_FLASH_WINDOW_SIZE;
    ...
    /* load the real ROM image (kept aside, served for unmapped window) */
    soc->rom = calloc(1, P4_ROM_SIZE);
    assert(soc->rom);
    ...
    /* The IDF bootloader dereferences the flash-chip struct through the
     * legacy-data pointer at 0x4FF3FFE8 (its own link address for
     * rom_spiflash_legacy_data). Point it at the ROM-maintained struct
     * (0x4FFBFEE8: device_id/chip_size kept current by the ROM and the
     * bootloader's config_param call), as on any working system. */
    {
        esp32_region_t *sram = esp32_find_region(soc, P4_SRAM_BASE);
        uint32_t ptr = 0x4FFBFEE8u;
...
#define P4_GPIO_INTR_SOURCE 74u
#define P4_RMT_INTR_SOURCE 43u
/* I2C0 interrupt source (interrupts.h enum) */
#define P4_I2C_EXT0_INTR_SOURCE 44u
/* TIMG0/TIMG1 timer interrupt sources (interrupts.h enum) */
#define P4_TG0_T0_INTR_SOURCE 46u
#define P4_TG1_T0_INTR_SOURCE 49u
/* TIMG0/TIMG1 watchdog interrupt sources (interrupts.h enum) */
#define P4_TG0_WDT_INTR_SOURCE 48u
#define P4_TG1_WDT_INTR_SOURCE 51u
/* PCNT interrupt source (interrupts.h enum) */
#define P4_PCNT_INTR_SOURCE 111u
/* MCPWM0 (PWM0) interrupt source (interrupts.h enum) */
#define P4_MCPWM_INTR_SOURCE 38u
/* I2S0 and GDMA (AHB_PDMA) interrupt sources (interrupts.h enum) */
#define P4_I2S0_INTR_SOURCE 27u
#define P4_DMA_IN_CH0_INTR_SOURCE 56u
#define P4_DMA_OUT_CH0_INTR_SOURCE 59u
...
#define AES_BASE        (P4_PERIPH_BASE + 0x88000u)
#define SHA_BASE        (P4_PERIPH_BASE + 0x89000u)
...
#define ETS_AES_INTR_SOURCE 69u /* from esp32p4 interrupts.h */
#define ETS_SHA_INTR_SOURCE 70u /* from esp32p4 interrupts.h */
...
/* MWDT (within TIMG) register offsets and bits (esp32p4 timer_group) */
...
static uint64_t esp32p4_wdt_cycles(esp32p4_t *soc, int g)
{
    uint32_t prescale =
...
static void esp32_uart_putc(esp32p4_t *soc, char c)
{
    if (esp32p4_uart_output)
        esp32p4_uart_output(c);
    else
        putchar(c);
...
/* P4 MMU pages are fixed 64KB (SOC_MMU_PAGE_SIZE); there is no
 * PAGE_MODE register (+0x384 is MMU_POWER_CTRL, stored but ignored). */
static uint32_t esp32_mmu_page_shift(esp32p4_t *soc)
{
    (void) soc;
    return 16u;
}
...
/* ------------------------------------------------------------------ */
/* P4 guest address translation                                      */
/* ------------------------------------------------------------------ */

/* P4 HP/LP peripheral address -> C6-style MMIO offset (for the shared
 * dispatch below, which is written against the 0x60000000 layout).
 * Returns ~0u when the address has no C6-style counterpart (handled by
 * the P4-specific blocks or the default zero path). Only the sub-ranges
 * the dispatch covers are mapped. */
static uint32_t p4_xlate(uint32_t addr)
{
    if (addr >= 0x500CA000u && addr < 0x500CC000u)
        return addr - 0x500CA000u + 0x0000u; /* UART0/1 */
    if (addr >= 0x5008C000u && addr < 0x5008E000u)
        return addr - 0x5008C000u + 0x2000u; /* FLASH_SPI0/1 */
    if (addr >= 0x500C4000u && addr < 0x500C5000u)
        return addr - 0x500C4000u + 0x4000u; /* I2C0 */
    if (addr >= 0x500A2000u && addr < 0x500A2200u)
        return addr - 0x500A2000u + 0x6000u; /* RMT (layout differs; stub) */
    if (addr >= 0x500D3000u && addr < 0x500D3200u)
        return addr - 0x500D3000u + 0x7000u; /* LEDC */
    if (addr >= 0x500C2000u && addr < 0x500C2100u)
        return addr - 0x500C2000u + 0x8000u; /* TIMG0 */
    if (addr >= 0x500C3000u && addr < 0x500C3100u)
        return addr - 0x500C3000u + 0x9000u; /* TIMG1 */
    if (addr >= 0x500E2000u && addr < 0x500E3000u)
        return addr - 0x500E2000u + 0xA000u; /* SYSTIMER */
    ...
    if (addr >= 0x50090000u && addr < 0x50090100u)
        return addr - 0x50090000u + 0x88000u; /* AES */
    if (addr >= 0x50091000u && addr < 0x50092000u)
        return addr - 0x50091000u + 0x89000u; /* SHA */
    if (addr >= 0x500E0000u && addr < 0x500E1000u)
        return addr - 0x500E0000u + 0x91000u; /* GPIO */
    return ~0u;
}
...
static uint32_t esp32_mmio_read(riscv_t *rv, esp32p4_t *soc, uint32_t addr)
{
    ...
    if (addr >= 0x500E6000u && addr < 0x500E7000u) {
        ...
        if (addr == 0x500E6004u)
             ...
             * The SDK sets it then busy-waits for it to clear. */
            v &= ~0x10u;
        return v;
    }
     ...
     * SYNC_CTRL (0x98) bit 4 = SYNC_DONE; the *_AUTOLOAD_CTRL regs
    ...
    if (addr >= 0x3FF10000u && addr < 0x3FF10400u) {
        ...
        if (off == 0xE4u || off == 0xF8u || off == 0x134u || off == 0x2B4u)
            v |= 0x2u; /* AUTOLOAD_DONE */
        if (off == 0x2A8u)
            v |= 0x2u; /* PRELOAD_DONE */
        if (off == 0x88u)
            v |= 0x4u; /* LOCK_DONE */
        if (off == 0x28Cu)
            /* L2_FREEZE_CTRL: FREEZE_DONE (bit 22) follows FREEZE_EN
             * (bit 20); the operation completes instantly. */
            v = (v & ~(1u << 22)) | (((v >> 20) & 1u) << 22);
        return v;
    }
    ...
    /* Translate P4 peripheral addresses to the shared C6-style layout. */
    if (addr >= P4_MMIO_BASE && addr < P4_MMIO_BASE + P4_MMIO_SIZE) {
        uint32_t t = p4_xlate(addr);
        if (t == ~0u)
            return 0; /* clocks/resets/LP regs: report idle defaults */
        addr = P4_PERIPH_BASE + t;
    }
     ...
     * CLINT-U (0x20001C00) */
    if (addr >= P4_PLIC_BASE && addr < P4_PLIC_BASE + P4_PLIC_SIZE) {
        uint32_t off = addr - P4_PLIC_BASE;
    ...
    if (addr >= P4_PLIC_CPU_BASE && addr < P4_PLIC_CPU_BASE + P4_PLIC_CPU_SIZE) {
        uint32_t off = addr - P4_PLIC_CPU_BASE;
        if (off >= 0x404u && off < 0x404u + 28 * 4u)
            return soc->plic_prio[(off - 0x404u) >> 2];
        return 0;
    }
    if (addr < P4_PERIPH_BASE || addr >= P4_PERIPH_BASE + P4_PERIPH_SIZE)
        return 0; /* unmapped: bus returns zeros */
    uint32_t off = addr - P4_PERIPH_BASE;
    uint32_t *mmio32 = (uint32_t *) soc->mmio;

    /* SYSTIMER_UNIT0_OP_REG (translated 0x6000a004): the mask ROM / SDK
     * issues a unit update (bit 30) and busy-waits on VALUE_VALID (bit 29).
     * Report it valid so the update completes instantly. */
    if (addr == 0x6000a004u)
    ...
    if (addr >= P4_PERIPH_BASE + 0xB000u &&
        addr < P4_PERIPH_BASE + 0xB100u) {
        uint32_t off = addr - P4_PERIPH_BASE - 0xB000u;
    ...
    if (addr >= P4_PERIPH_BASE + 0x12000u &&
        addr < P4_PERIPH_BASE + 0x12100u) {
    ...
    if (addr >= P4_PERIPH_BASE + 0x14000u &&
        addr < P4_PERIPH_BASE + 0x14130u) {
    ...
    if (addr < P4_PERIPH_BASE + 0x4000u) {
        ...
        if (addr == P4_PERIPH_BASE + 0x2000u + SPI_AXI_STATUS_REG)
            return mmio32[off >> 2] | 0x80000000u; /* MSPI idle */

        if (addr == P4_PERIPH_BASE + 0x2000u + 0x178u)
            /* SMEM_AXI_ADDR_CTRL: ALL_FIFO_EMPTY (26) + RDATA/RADDR/
             * WDATA/WBLEN/ALL_AXI_TRANS_AFIFO_EMPTY (27-31) default 1:
             * all AXI/SPI0 transfers are done. The ROM waits for bit 31
             * after closing the AXI path. */
            return mmio32[off >> 2] | 0xFC000000u;

        if (addr == P4_PERIPH_BASE + 0x202Cu)
    ...
    if (addr >= P4_PERIPH_BASE + 0x7000u &&
        ...
        uint32_t o = addr - P4_PERIPH_BASE - 0x7000u;
        for (int c = 0; c < 6; c++)
    ...
    if (addr >= P4_PERIPH_BASE + 0x8000u &&
        addr < P4_PERIPH_BASE + 0x8100u) {
        uint32_t o = addr - P4_PERIPH_BASE - 0x8000u;
        if (o == TIMG_T0LO)
    ...
    if (addr >= P4_PERIPH_BASE + 0x9000u &&
        addr < P4_PERIPH_BASE + 0x9100u) {
        uint32_t o = addr - P4_PERIPH_BASE - 0x9000u;
        if (o == TIMG_T0LO)
    ...
    /* GPIO */
    if (addr >= P4_PERIPH_BASE + 0x91000u && addr < P4_PERIPH_BASE + 0x92000u) {
        switch (off - 0x91000u) {
        case GPIO_OUT_REG:
            return soc->gpio_out;
        ...
        case GPIO_IN_REG:
            return soc->gpio_in | esp32p4_gpio_eff_out(soc, mmio32);
...
static void esp32p4_aes_run_dma(riscv_t *rv, int ch_out)
{
    ...
    uint32_t in_cur = P4_SRAM_BASE + (soc->gdma_out_link[ch_out] & 0xFFFFFu);
    uint32_t out_cur = ch_in >= 0 ? P4_SRAM_BASE + (soc->gdma_in_link[ch_in] & 0xFFFFFu) : 0;
    ...
    for (;;) {
        if (in_len == 0) {
            if (!in_cur) break;
            uint8_t *d = esp32p4_dma_ptr(soc, in_cur);
            if (!d) break;
            uint32_t dw0, dw1, dw2;
        ...
        for (int i = 0; i < 16; i++) {
            uint8_t *p = esp32p4_dma_ptr(soc, in_buf + in_off);
        ...
        for (int i = 0; i < 16; i++) {
            if (out_len == 0) {
                if (!out_cur) break;
                uint8_t *d = esp32p4_dma_ptr(soc, out_cur);
                if (!d) break;
                uint32_t dw0, dw1, dw2;
                memcpy(&dw0, d, 4); memcpy(&dw1, d + 4, 4); memcpy(&dw2, d + 8, 4);
            ...
            uint8_t *p = esp32p4_dma_ptr(soc, out_buf + out_off);
            if (p) *p = out[i];
            out_off++; out_len--;
        }
    }
    ...
    for (uint32_t w = out_desc0; w;) {
        uint8_t *d = esp32p4_dma_ptr(soc, w);
        if (!d) break;
        uint32_t dw0, dw1, dw2;
        memcpy(&dw0, d, 4); memcpy(&dw1, d + 4, 4); memcpy(&dw2, d + 8, 4);
...
static void esp32p4_gpio_edge_check(esp32p4_t *soc, uint32_t *mmio32,
                                    uint32_t new_live);
                              ...
                              unsigned size)
{
    ...
    if (addr >= 0x50115000u && addr < 0x50116000u) {
        soc->pmu_reg[(addr - 0x50115000u) >> 2] = val;
        return;
    }
    ...
    /* P4 cache control (0x3FF10000): plain storage. */
    if (addr >= 0x3FF10000u && addr < 0x3FF10400u) {
        soc->cache_reg[(addr - 0x3FF10000u) >> 2] = val;
        return;
    }
    ...
    /* LP_TIMER (0x50112000): the firmware arms a sleep timer by writing the
     * target (TAR0_LO @0x00, TAR0_HI @0x04, enable in bit 31 of hi)
     * and then busy-waits on the free-running counter (BUF0_LO @0x14,
     * BUF0_HI @0x18). On each UPDATE pulse (0x10, bit 28) we advance the
     * counter; once it reaches the target we latch the SOC_WAKEUP_INT_RAW
     * bit (INT_RAW @0x28, bit 31) so the wakeup-wait loop exits. */
    if (addr >= 0x50112000u && addr < 0x50113000u) {
        uint32_t off = addr - 0x50112000u;
        if (off == 0x00u) {
    ...
    if (addr >= P4_MMIO_BASE && addr < P4_MMIO_BASE + P4_MMIO_SIZE) {
        uint32_t t = p4_xlate(addr);
        if (t == ~0u)
            return; /* clocks/resets/LP regs: accept and ignore */
        addr = P4_PERIPH_BASE + t;
    }
    if (addr < P4_PERIPH_BASE || addr >= P4_PERIPH_BASE + P4_PERIPH_SIZE)
    ...
    if (addr >= P4_PERIPH_BASE + 0x4000u &&
        ...
        } else if (addr == P4_PERIPH_BASE + 0x401cu) { /* data: TX fifo push */
        ...
        if (addr == P4_PERIPH_BASE + 0x4080u) {
        ...
        if (addr >= base && addr < base + 0x1000u) {
            ...
            if (o == UART_FIFO_REG) {
                ...
                *raw &= ~val;
                 ...
                 * ISR. The P4 Arduino core does not always arm these bits in
    ...
    if (addr < P4_PERIPH_BASE + 0x4000u) {
        ...
        if (addr >= P4_PERIPH_BASE + 0x2000u + SPI_MMU_ITEM_CONTENT &&
            ...
            case SPI_MMU_ITEM_INDEX:
                ...
                return;
            case SPI_MMU_PAGE_MODE:
                /* +0x384 is MMU_POWER_CTRL on P4 (no PAGE_MODE register;
                 * pages are fixed 64KB): store, ignore for paging. */
    ...
    if (addr >= P4_PERIPH_BASE + 0x91000u && addr < P4_PERIPH_BASE + 0x92000u) {
        ...
        esp32p4_gpio_edge_check(soc, mmio32,
            soc->gpio_in | esp32p4_gpio_eff_out(soc, mmio32));
        if (esp32p4_gpio_output) {
            for (int pin = 0; pin < 30; pin++) {
                if (soc->gpio_enable & (1u << pin))
                    esp32p4_gpio_output(pin, !!(soc->gpio_out & (1u << pin)));
            }
        }
        return;
    }
    ...
    /* (P4 cache at 0x3FF10000 and the crosscore flag at 0x500E5010 are
     * handled by the prologue/default paths; the C6 ICache/INTPRI
     * absolute blocks do not apply.) */
    mmio32[off >> 2] = val;
}
...
{
    return esp32_find_region(PRIV(rv)->esp32p4, addr);
}

/* Flash cache window translation: the 0x40000000 window (64 MB) maps
...
static inline uint32_t esp32_flash_window_off(esp32p4_t *soc, uint32_t addr)
{
    if (addr >= P4_FLASH_I_BASE && addr < P4_FLASH_I_BASE + P4_FLASH_WINDOW_SIZE) {
        ...
        uint32_t page = (addr - P4_FLASH_I_BASE) >> shift;
        if (page >= 1024)
            return ~0u;
        uint32_t fpage = soc->mmu[page];
        if (fpage == ~0u)
            return ~0u; /* never programmed: ROM fallback / zero */
        /* P4 MMU entry: physical page = bits[10:0] (mask 0x7FF),
         * VALID = bit 12 (SOC_MMU_FLASH_VALID). Entries without VALID
         * (e.g. mmu_hal_init's reset pattern of 0) are unmapped. */
        if (!(fpage & 0x1000u))
            return ~0u;
        uint32_t off = ((fpage & 0x7FFu) << shift) | (addr & mask);
        return (off < P4_FLASH_SIZE) ? off : ~0u;
    }
    return ~0u;
}
...
uint16_t esp32p4_read_s(riscv_t *rv, uint32_t addr)
{
    esp32_region_t *r = esp32_lookup(rv, addr);
    if (!r || r->type != ESP32_REG_RAM)
        return (uint16_t) esp32_mmio_read(rv, PRIV(rv)->esp32p4, addr);
    if (r->base == P4_FLASH_I_BASE)
        return (uint16_t) esp32_window_read(PRIV(rv)->esp32p4, r->data, addr, 2);
    uint16_t val;
    memcpy(&val, r->data + (addr - r->base), 2);
    return val;
}
...
void esp32p4_write_w(riscv_t *rv, uint32_t addr, uint32_t val)
{
    esp32_region_t *r = esp32_lookup(rv, addr);
    if (!r || r->type != ESP32_REG_RAM) {
        esp32_mmio_write(rv, addr, val, 4);
        return;
    }
    if (r->base == P4_FLASH_I_BASE)
...
void esp32p4_write_s(riscv_t *rv, uint32_t addr, uint16_t val)
{
    esp32_region_t *r = esp32_lookup(rv, addr);
    if (!r || r->type != ESP32_REG_RAM) {
        esp32_mmio_write(rv, addr, val, 2);
        return;
    }
    if (r->base == P4_FLASH_I_BASE)
        return; /* flash window is RX; ignore writes */
...
static uint32_t esp32p4_translate(riscv_t *rv, uint32_t vaddr, bool rw)
{
    (void) rv;
    (void) rw;
...
{
    riscv_io_t io = {
        .mem_ifetch = esp32p4_ifetch,
        .mem_read_w = esp32p4_read_w,
        .mem_read_s = esp32p4_read_s,
        .mem_read_b = esp32p4_read_b,
        .mem_write_w = esp32p4_write_w,
        .mem_write_s = esp32p4_write_s,
        .mem_write_b = esp32p4_write_b,
        .mem_translate = esp32p4_translate,
        .mmu_read_w = esp32p4_read_w,
        .mmu_read_s = esp32p4_read_s,
        .mmu_read_b = esp32p4_read_b,
...
uint32_t esp32p4_boot(esp32p4_t *soc, const char *elf_path)
{
    ...
    if (esp32p4_flash_image_path) {
        FILE *f = fopen(esp32p4_flash_image_path, "rb");
        if (!f) {
            fprintf(stderr, "esp32p4: cannot open flash image %s\n",
                    esp32p4_flash_image_path);
            exit(EXIT_FAILURE);
        }
        esp32_region_t *fd = esp32_find_region(soc, P4_FLASH_I_BASE);
    ...
    if (!f) {
        fprintf(stderr, "esp32p4: cannot open %s\n", elf_path);
        exit(EXIT_FAILURE);
    }
    esp32_elf32_hdr_t hdr;
    if (fread(&hdr, 1, sizeof(hdr), f) != sizeof(hdr) ||
        memcmp(hdr.e_ident, "\x7f" "ELF", 4) != 0) {
        fprintf(stderr, "esp32p4: %s is not an ELF32 file\n", elf_path);
        exit(EXIT_FAILURE);
    }
    for (int i = 0; i < hdr.e_phnum; i++) {
        ...
        for (uint32_t pos = 0; pos < sz;) {
            ...
            if (!r || r->type != ESP32_REG_RAM) {
                fprintf(stderr,
                        "esp32p4: ELF segment at 0x%08x outside RAM\n",
                        vaddr + pos);
                exit(EXIT_FAILURE);
            }
            ...
            if (copy) {
                if (fseek(f, ph.p_offset + pos, SEEK_SET) ||
                    fread(r->data + (vaddr + pos - r->base), 1, copy, f) !=
                        copy) {
                    fprintf(stderr, "esp32p4: ELF segment read failed\n");
                    exit(EXIT_FAILURE);
                }
            }
            pos += chunk;
...
void esp32p4_check_interrupt(riscv_t *rv)
{
     ...
     * ATTR bit0 misroutes every configured line to the mtvec base panic.)
     * The P4 MTVT holds handler ADDRESSES (not jump stubs): load the entry
     * and vector to it. */
    {
        uint8_t *vp = esp32p4_guest_to_host(soc, rv->csr_mtvt + 4u * (uint32_t) best_id);
...
static void esp32p4_uart_rx_poll(esp32p4_t *soc)
{
    if (!esp32p4_uart_rx_path)
        return;
    if (soc->uart_rx_fd < 0) {
        soc->uart_rx_fd =
            open(esp32p4_uart_rx_path, O_RDONLY | O_NONBLOCK);
        if (soc->uart_rx_fd < 0) {
            if (errno != ENOENT && errno != EACCES)
                fprintf(stderr, "esp32p4: uart rx open %s: %s\n",
                        esp32p4_uart_rx_path, strerror(errno));
            return;
        }
    }
    for (;;) {
...
static void esp32p4_mcpwm_fire(esp32p4_t *soc, int t, int dir,
...
void esp32p4_periodic(riscv_t *rv)
{
    ...
    for (int ch = 0; ch < 3; ch++) {
        ...
        if (!soc->gdma_rx_next_addr[ch]) {
            ...
            esp32p4_gdma_load_rx_desc(soc, ch);
        }
    }
    ...
    */
     ...
     * MCPWM signal (89..94 = PWM0_OUT{0,1,2}{A,B}). */
    {
        ...
        for (int t = 0; t < 3; t++) {
            ...
            for (uint64_t i = 0; i < elapsed; i++) {
                ...
                esp32p4_mcpwm_fire(soc, t, soc->mcpwm_dir[t], phase, period);
                if (mod == 1u) { /* up */
                    soc->mcpwm_dir[t] = 0;
    ...
    for (int g = 0; g < 2; g++) {
        ...
        uint32_t src = (g == 0) ? P4_TG0_T0_INTR_SOURCE
                                : P4_TG1_T0_INTR_SOURCE;
